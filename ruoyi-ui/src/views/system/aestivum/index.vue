    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:aestivum:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:aestivum:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:aestivum:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:aestivum:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="aestivumList" @selection-change="handleSelectionChange">
...
<script>
import { listAestivum, getAestivum, delAestivum, addAestivum, updateAestivum } from "@/api/system/aestivum";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAestivum(pathway).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.pathway != null) {
            updateAestivum(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAestivum(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除第二个编号为"' + pathways + '"的数据项？').then(function() {
        return delAestivum(pathways);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/aestivum/export', {
        ...this.queryParams
      }, `aestivum_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
